...
module.exports = class LogicaNegocio {
    ...
    *  cuantas: N,
     ...
     * 
     */
    async guardarSensor(sensor, correo){
        try {

            //Comprobamos si el sensor ya está registrado
            var sensorBuscado = await this.buscarSensor(sensor.macSensor);
            ...
            if(sensorBuscado == 404){
                ...
                if( sensor.macSensor && sensor.tipoMedicion && correo){
    ...
    async registrar(datosUsuario, datosSensor){
        try{
            ...
            if(usuarioRegistrado == 200){
                var sensorRegistrado = await this.guardarSensor(datosSensor, datosUsuario.correo);
